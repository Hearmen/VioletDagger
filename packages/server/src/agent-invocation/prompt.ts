...
**结束本次会话**：你这次是一次性非交互调用，没有人类终端可以应答。正式发现先通过 post_message 提交并等待成功；探索完成时调用 complete_exploring；完成本次工作后就结束本次回答，由系统在进程自然退出后记录本次 session。不要等待终端输入。什么都不调用直接结束是允许的，不会被视为异常，只是这次没有新内容。
